...
export const getConditions = async (accessToken: string) => {
  try {
    const res = await backend.get<{ data: ConditionType[]; error: boolean }>(
      "/conditions",
      {
        headers: {
          Authorization: `Bearer ${accessToken}`,
    ...
    return handleResponseBack<ConditionType[]>(res);
  } catch (error) {
    return handleErrorBack<ConditionType[]>(error);
  }
};

export const getConditionById = async (accessToken: string, id: string) => {
  try {
    const res = await backend.get<{ data: ConditionType; error: boolean }>(
      `/conditions/${id}`,
      {
...
export const createCondition = async (props: {
  accessToken: string;
  condition: ConditionPayloadType;
}) => {
  try {
    const res = await backend.post<{ data: ConditionType; error: boolean }>(
      "/conditions",
      props.condition,
      {
        headers: {
